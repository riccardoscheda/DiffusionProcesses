...
def evo(frames):
    ...
    maxwellpdf = np.sqrt(m/(2*np.pi*KT))*np.exp(-x**2*m/(2*KT))
    trajectory.set_data(x, maxwellpdf)
    particle.set_data(binp[:-1], rhop)
    ...
    maxwellpdf = 2*np.sqrt(m/(2*np.pi*KT))*np.exp(-x**2*m/(2*KT))

    maxwelldist.set_data(hist[1][:-1],hist[0])
